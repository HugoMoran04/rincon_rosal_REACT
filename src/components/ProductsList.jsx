        ...
        <>
          ...
          {numSelected === 1 && (
            <Tooltip title="Editar" >
              <IconButton  onClick={() => navigate(`/productForm/${selected[0]}`)}//</Tooltip>onClick={() => setEditingId(selected[0])}
              >
                <BorderColorIcon />
              </IconButton>
            </Tooltip>
...
function ProductsList() {
  ...
  const [cateogrias, setCategorias] = useState([]); 
  const navigate = useNavigate();
  ...
  useEffect(() => {
    async function fetchCategorias() {
      try {
        ...
        console.error("Error de conexión al obtener las categorías:", error);
      }
    }
  ...
  const rowsConCategoria = useMemo(() => {
    return rows.map((producto) => {
      const categoria = cateogrias.find(
        (cat) => cat.id_categoria === producto.id_categoria
      );
      return {
        ...producto,
        nombre_categoria: categoria ? categoria.nombre : "Sin categoría",
      };
    });
  }, [rows, cateogrias]);
  
  const handleDeleteConfirm = async () => {
    if (!productToDelete.length) return;

    try {
      for (const id_producto of productToDelete) {
        let response = await fetch(apiUrl + "/productos/" + id_producto, {
          method: "DELETE",
        });
        if (!response.ok) {
          alert("Error al eliminar el producto con ID: " + id_producto);
        }
    }

    setRows(prev => prev.filter(producto => !productToDelete.includes(producto.id_producto)));
    setSelected([]);
    handleCloseModal();
    } catch (error) {
 ...
 /* const handleDelete = async (id) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.id_producto);
  ...
  const visibleRows = useMemo(
    ...
    [rowsConCategoria, order, orderBy, page, rowsPerPage]
  );
  useEffect(() => {
  ...
  const handleOpenModal = (ids) => {
    setProductToDelete(Array.isArray(ids) ? ids : [ids]);
    setOpenModal(true);
  };
  ...
  return (
          ...
          >
            ...
            <TableBody>
              {visibleRows.map((row, index) => {
                const isItemSelected = selected.includes(row.id_producto);
    ...
    onClick={() => navigate("/productForm")} // tu ruta al otro componente
